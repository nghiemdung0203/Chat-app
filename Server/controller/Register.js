...
module.exports.Register = async (req, res) => {
  ...
  await cloudianry.uploader.upload(avatarpath, {folder: 'chat-app'}).then(async(result) => {
    fs.unlinkSync(avatarpath);
    await client
    .execute("INSERT INTO users (user_id, avatar, Email, password, salt) VALUES (?, ?, ?, ?, ?)", [
      user_id,
      result.url,
      Email,
      passHash,
    ...
    .then(async(ressu) => {
      await client.execute('SELECT * from users WHERE Email = ? ALLOW FILTERING', [Email]).then((ress) => {
        let token = jwt.sign(
          { user_id: ress.rows[0].user_id, Email , Avatar: ress.rows[0].avatar},
